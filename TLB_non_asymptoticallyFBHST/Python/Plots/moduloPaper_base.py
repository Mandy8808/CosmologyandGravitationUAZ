...
def U0(phiMax, argU0, U0S, sig, Nptos=1000, Rtol=1e-21, Atol=1e-23, fStep=1e-10):
    """
    ...
    U01 = fsolve(func, U0S)[0]  # u(pi/2) donde du=0
    du = 1e-05  # delta para moverlo del pto de equilibrio
    ...
    sol1 = solve_ivp(eqSchdSitter, [phiMin, phiMax], [U01-du], args=(arg,), t_eval=thetaspan,
                     first_step=fStep, method='RK45', rtol=Rtol, atol=Atol)
    u0 = sol1.y[0][-1]
    phi0 =  sol1.t[-1]
...
# Resolución
def integracion(paramF, geoDat, DatLim, Nptos=1000, Rtol=1e-21, Atol=1e-23, fStep=1e-10): # , 
    """
    ...
    #uC = fsolve(func, u0)[0] if u0<1 else fsolve(func, u0*1e06)[0]
    uC = fsolve(func, u0)[0]  # uC donde du=0  uC = fsolve(func, u0)[0]
    ...
    sol1 = solve_ivp(eqSchdSitter, [PhiMin, PhiMax], [u0], args=(arg,), t_eval=thetaspan, first_step=fStep,
                     events=ucrit, method='Radau', rtol=Rtol, atol=Atol) #Radau
    
    ### Checking that b3 is not equivalent to inf 
    #print('evento', sol1.t_events, sol1.t[-1])

    ##### Integración a partir de uC
    u0 = sol1.y[0][-1] # Condición inicial
    PhiMin = sol1.t[-1]
    thetaspan = np.linspace(PhiMin, PhiMax, Nptos)
    ...
    solFphi = np.concatenate((sol1.t, sol2.t))
    solFu = np.concatenate((sol1.y[0]/fac, sol2.y[0]/fac))
    return solFphi, solFu


def Intind(paramF, Val_b, delta, Nptos=1000,
               Rtol=1e-21, Atol=1e-23, fStep=1e-10):
    """
    ...
    for i in [0, 1, 2]: # 0 -> HORIZONTAL, 1 -> PENDIENTE POSITIVA, 2 -> PENDIENTE NEGATIVA
        ...
        solFphi, solFu = integracion(paramF, geoDat, DatLim, Nptos=Nptos, Rtol=Rtol, Atol=Atol, fStep=fStep)
        DatGeo.append([solFphi, solFu])

    return DatGeo


def triang(paramF, Val_b, delta, Nptos=1000,
           Rtol=1e-21, Atol=1e-23, fStep=1e-10):
    """
    se construye el triángulo
    Vectores de entrada
...
def AngulP(dat, paramF, b, ptos):
    ...
    tanF = interp1d(phi, tan, kind='quadratic')
...
def AngSchDSitter(paramF, Val_b, delta, Datptos):
    ...
    dc1 = np.gradient(u1, phi1) # C1
    dc2 = np.gradient(u2, phi2) # C2    
    dc3 = np.gradient(u3, phi3) # C3
    ...
    # Internal angles
    #beta1 = angC1[0]-angC3[1]
    #beta2 = angC2[0]-angC1[1]
    #beta3 = np.pi-angC2[1]+angC3[0]
